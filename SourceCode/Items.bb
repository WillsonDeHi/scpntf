...
Global ItemAmount%
Global Inventory.Items[MaxItemAmount] ; TODO make MaxItemAmount dynamic and establish a "inventory size" value that will cap the usable portion of the array
...
Function CreateItemTemplate.ItemTemplates(name$, tempname$, objpath$, invimgpath$, imgpath$, scale#, texturepath$ = "",invimgpath2$="",Anim%=0, texflags%=9)
	...
	If texturepath <> "" Then
		For it2.itemtemplates = Each ItemTemplates
			...
			EndIf
		Next
		If texture=0 Then texture=LoadTexture_Strict(texturepath,texflags%,0) : it\texpath = texturepath; : DebugLog texturepath
...
Function InitItemTemplates()
	...
	it = CreateItemTemplate("Note", "paper", "GFX\items\paper.x", "GFX\items\INVnote.jpg", "GFX\items\docRAND2.jpg", 0.003, "GFX\items\notetexture.jpg") : it\sound = 0 
	...
	it\sound = 2
	it\fastPickable = True
	it = CreateItemTemplate("Bulky Ballistic Vest", "veryfinevest", "GFX\items\vest.b3d", "GFX\items\INVvest.jpg", "", 0.025,"GFX\items\Vest.png")
	it\sound = 2
	...
	it = CreateItemTemplate("Coin", "coin", "GFX\items\key.b3d", "GFX\items\INVcoinrust.jpg", "", 0.0005, "GFX\items\coinrust.png","",0,1+2+8) : it\sound = 3
	...
	;new Items in SCP:NTF 0.2.0 - ENDSHN
...
;	it\isGun% = True
;	it\sound = 66
;	it = CreateItemTemplate("Crowbar", "crowbar", "Unused\GFX\items\Crowbar_Worldmodel.b3d", "Unused\GFX\items\INVfreezer.jpg", "", 0.03)
;	it\isGun% = True
;	it\sound = 66
;	it = CreateItemTemplate("M9 Beretta", "m9", "Unused\GFX\items\M9_Beretta_Worldmodel.b3d", "Unused\GFX\items\INVfreezer.jpg", "", 0.02)
;	it\isGun% = True
;	it\sound = 66
	it = CreateItemTemplate("Document SCP-457 Page 1/2","paper","GFX\items\paper.x","GFX\items\INVpaper.jpg","GFX\items\doc457_1.jpg", 0.003) : it\sound = 0
...
;	CreateItemTemplate("Silencer", "silencer", "GFX\weapons\Silencer.b3d","GFX\weapons\INVsilencer.jpg","",0.02)
	...
	CreateItemTemplate("Broken Helmet", "brokenhelmet", "GFX\items\helmet_broken.b3d", "GFX\items\INVpaper.jpg", "", 0.116)
				...
				If Inventory[n] = Null Then
					Select item\itemtemplate\tempname
						...
						Case "hazmatsuit", "hazmatsuit2", "hazmatsuit3"
							...
							Next
						Case "brokenhelmet"
							Msg = GetLocalString("Items","brokenhelmet")
							MsgTimer = 70*6
							Exit
					End Select
...
;		Local specificGunInInv% = False
;		For n% = 0 To MaxItemAmount-1
;			If Inventory[n] <> Null
;				If Inventory[n]\itemtemplate\tempname = item\itemtemplate\tempname
;					specificGunInInv = True
;					Exit
;				EndIf
;			EndIf
;		Next
;		
;		If specificGunInInv
;			If GiveAmmo(item\itemtemplate\tempname)
;				PlaySound_Strict LoadTempSound("SFX\Guns\"+Inventory[n]\itemtemplate\tempname+"\pickup.ogg")
;				Msg = "You picked up a magazine for the "+item\itemtemplate\name
;				MsgTimer = 70 * 5
;				RemoveItem(item)
;			Else
		...
		For g = Each Guns
			If g\name = item\itemtemplate\tempname Then
				If g\GunType = GUNTYPE_MELEE And found Then
					Msg = "You already have a " + item\itemtemplate\name
					MsgTimer = 70 * 5
					Exit
				EndIf
				If (Not found) Then
