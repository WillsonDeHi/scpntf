...
Global ParticleTextures%[14] ; TODO this can be shrunk
...
Function UpdateEmitters()
	...
	For e.Emitters = Each Emitters
		...
		If FPSfactor > 0 And updateEmitter Then
			...
			EndIf
			;EndIf
			If e\emittertype<>2 And e\emittertype<>3
				e\SoundCHN = LoopSound2(HissSFX, e\SoundCHN, Camera, e\Obj)
			EndIf
			
			If InSmoke = False Then
				If (Not mpl\HasNTFGasmask) And (Not WearingGasMask) And (Not WearingHazmat) Then
	...
	If InSmoke Then
		...
		If EyeIrritation > (70 * 24) Then 
			DeathMSG = GetLocalStringR("Singleplayer", "smoke_death", Designation)
